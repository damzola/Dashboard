...
import { BarChart, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, Bar, LineChart, Line, Legend } from "recharts";
export default function dashboard() {
  ...
  return (
    <div className=' container bg-white'>
      ...
      <div className=' d-flex p-1 m-2 gap-4 overflow-x-scroll justify-content-center w-100 h-100'>
        {
          Data.map((item,id)=>(
            <div key={id} className='d-flex bg-light  '>
               <div  className='justify-content-center align-content-center p-1'>
              <h3>{item.value}</h3>
              {/* <h3>{item.earn}</h3>
              <h3>{item.order}</h3>
              <h3>{item.task}</h3>
              <h3>{item.success_task}</h3>
              <h3>{item.pending_task}</h3>
              <h3>{item.fail_task}</h3> */}
              <p>{item.word}</p>
        </div>
            </div>
        ...
        <h5 className='text-start mb-3'>Monthly Visits</h5>
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={value}>
              ...
              <Tooltip />
              <Legend/>
              <Bar dataKey="visits" fill="#008080" barSize={40} radius={[10, 10, 0, 0]} />
            </BarChart>
          </ResponsiveContainer>
        </div>
  ...
  <form className='row g-3'>
    ...
    <div className='col-12 w-100'>
      <button type='submit' className='btn button mt-2'>Submit</button>
    </div>
  </form>   
        </div>
         ...
         <div className='col-lg-6 col-md-6 bg-light shadow rounded-4 mt-2'>
          ...
          <ResponsiveContainer width="100%" height={300}> 
          <LineChart width={400} height={400} data={dataSet} margin={{ top: 5, right: 20, left: 10, bottom: 5 }}>
        <XAxis dataKey="name" />
        <Tooltip />
        <Legend/>
        <CartesianGrid strokeDasharray='3 3' />
        <Line type="monotone" dataKey="uv" stroke="#ff7300" yAxisId={0} activeDot={{ r: 8 }}/>
        <Line type="monotone" dataKey="pv" stroke="#387908" yAxisId={1} activeDot={{ r: 8 }}/>
        </LineChart>
        </ResponsiveContainer>
         </div>
</div>
